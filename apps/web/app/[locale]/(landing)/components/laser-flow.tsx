 ...
 *
 * A single volumetric laser beam with animated micro-streak wisps and
 * drifting fog, rendered with three.js. Hyperion adaptations, same
 * reasoning as the ogl-based WebGL components in this directory
 * (Lightfall, Galaxy, LightRays):
 *  - default color synced to the site's Platinum anchor (#EEEEED)
 *    instead of the original pink (#FF79C6) — this site's palette has
 *    no hue anywhere (pass your own `color` to override)
 *  - companion CSS folded into a Tailwind className on the wrapper
 *  - render loop pauses via IntersectionObserver + document
 *    visibility, same as the source's own perf-adaptive DPR logic
 *  - adaptive DPR/FPS throttling kept as-is (needed for three.js perf
...
export default function LaserFlow({
  ...
  useEffect(() => {
    ...
    const { r, g, b } = hexToRgb(color || "#EEEEED");
  ...
  return (
    <div
      className={`pointer-events-none relative size-full ${className}`.trim()}
      ref={mountRef}
      style={style}
    />
  );
